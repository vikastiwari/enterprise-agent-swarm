...
## Bug 37: "Swarm is thinking..." Loading State Bleeding
- **Symptom:** Submitting a prompt in one agent's chat window and immediately switching to another agent's window caused the "Swarm is thinking..." loading text to erroneously appear in the newly selected chat window.
- **Root Cause:** While the chat messages were isolated per-channel (Bug 36), the `isLoading` boolean was still tracked using a local React `useState` inside `ChatInterface.jsx`. Because the component doesn't unmount when the `activeChannel` prop changes, the true boolean state persisted visually across all channels.
- **Fix:** Lifted the loading state into the global Zustand store as `chatLoadingStates: {}` (a dictionary keyed by `channelId`). Exposed a `setChatLoading` action. `ChatInterface.jsx` now strictly derives its `isLoading` visual state from `chatLoadingStates[activeChannel]`, ensuring the loading indicator stays firmly locked to the channel that actually fired the API request.
